...
let buffer=[];
let timer=null;
function resetTimer(){ if(timer) clearTimeout(timer); timer=setTimeout(()=>processBatch().catch(e=>console.error(e)), BATCH_INTERVAL); }
async function processBatch(){
  ...
  try {
    ...
    const recipients=[]; const metadataHashes=[]; const contentTypes=[];
    ...
    const tx = await contract.batchMint(recipients, metadataHashes, contentTypes, { gasLimit: 5_000_000 });
